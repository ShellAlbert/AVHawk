...
ZHardEncThread::ZHardEncThread(QList<ZVideoThread*> *threadList,QMutex *mutex)
{
...
void ZHardEncThread::ZBindInFIFO(QQueue<QByteArray*> *freeQueue,QQueue<QByteArray*> *usedQueue,///<
...
qint32 ZHardEncThread::ZStartThread()
{
    this->start();
    return 0;
}
qint32 ZHardEncThread::ZStopThread()
{
    this->quit();
    this->wait(1000);
    return 0;
}
bool ZHardEncThread::ZIsExitCleanup()
{
    return this->m_bCleanup;
}
void ZHardEncThread::ZDoCleanBeforeExit()
{
    //set global request exit flag to notify other threads to exit.
    gGblPara.m_bGblRst2Exit=true;
    this->m_bCleanup=true;
    emit this->ZSigFinished();
    qDebug()<<"<MainLoop>:MainVideoTxThread ends.";
}
void ZHardEncThread::run()
{
    cpu_set_t cpuSet;
    CPU_ZERO(&cpuSet);
    CPU_SET(5,&cpuSet);
    ...
    //initial global sps/pps variable.
    gGblPara.m_video.m_nSpsPpsLen=0;
    gGblPara.m_video.m_pSpsPspBuffer=new qint8[BUFSIZE_1MB];


    //init RK3399 MPP for H264 encodec.
    MPP_RET ret = MPP_OK;
    ...
    if (p->type == MPP_VIDEO_CodingAVC)
    {
        ...
        if (packet)
        {
            ...
            //save sps/pps to global variable.
            memcpy(gGblPara.m_video.m_pSpsPspBuffer,ptr,len);
            gGblPara.m_video.m_nSpsPpsLen=len;
    ...
    qDebug()<<"<MainLoop>:MainVideoTxThread starts.";
    ...
    while(!gGblPara.m_bGblRst2Exit)
    {
        ...
        if(packet)
        {
            ...
            //add h264 frames to other threads' fifo.
            this->m_mutex->lock();
            for(qint32 i=0;i<this->m_threadList->size();i++)
            {
                bool bBypassIt=false;
                ZVideoThread *thread=this->m_threadList->at(i);
                ...
                ZVideoPkt *pkt=thread->m_freeQueueIn.dequeue();
                memcpy(pkt->m_baData->data(),pH264Data,nH264DataLen);
                pkt->m_nDataLen=nH264DataLen;
