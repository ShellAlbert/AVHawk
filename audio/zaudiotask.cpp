...
ZAudioTask::~ZAudioTask()
{
    ...
    if(this->m_encThread!=NULL)
    {
        delete this->m_encThread;
    }
    if(this->m_audioServer!=NULL)
    {
        delete this->m_audioServer;
    }
}
qint32 ZAudioTask::ZStartTask()
{
    ...
    //                                          -- tx queue --> Tcp Tx.
    this->m_capThread=new ZAudioCaptureThread(gGblPara.m_audio.m_capCardName);
    ...
    //create audio server.
    this->m_audioServer=new ZAudioServer(TCP_PORT_AUDIO,&this->m_mutex);

    //tcp tx thread.
    this->m_encThread=new ZAudioEncThread(&this->m_audioServer->m_threadList,&this->m_mutex);
    this->m_encThread->ZBindFIFO(&this->m_Ns2TxFIFOFree,&this->m_Ns2TxFIFOUsed,&this->m_Ns2TxFIFOMutex,&this->m_condNs2TxFIFOEmpty,&this->m_condNs2TxFIFOFull);
    QObject::connect(this->m_encThread,SIGNAL(ZSigThreadFinished()),this,SLOT(ZSlotHelpThreads2Exit()));


    //start audio tcp server.
    if(this->m_audioServer->ZStartServer()<0)
    {
        qDebug()<<"failed to start audio server.";
        return -1;
    }
    //start threads.
