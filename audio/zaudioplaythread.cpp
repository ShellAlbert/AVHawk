...
*/
    ...
    */
    ...
    while(!gGblPara.m_bGblRst2Exit)
    {
        ...
        //2.used buffer to play one frame.
        while(1)
        {
            qint32 nRet=snd_pcm_writei(this->m_pcmHandle,bufferIn->data(),PERIOD_SIZE>>2);
            if(nRet<0)
            {
                if(nRet==-EPIPE)
                {
                    qDebug()<<"<AudioPlayback>:Buffer Underrun.";
                ...
                //>0,a positive number of frames actually written.
                break;
            }
        }

        //3.put non-valid buffer to fifo.
        this->m_mutex->lock();
