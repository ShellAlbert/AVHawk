...
#include <QDateTime>
ZAudioCaptureThread::ZAudioCaptureThread(QString capDevName)
{
...
void ZAudioCaptureThread::run()
{
    ...
    */
    ...
    {
        //1.get a free buffer from fifo.
        bool bWaitTimeoutPlusGblRst2Exit=false;
        this->m_mutex->lock();
        while(this->m_freeQueue->isEmpty())
        {
            if(!this->m_condQueueEmpty->wait(this->m_mutex,5000))//timeout 5s to check exit flag.
            {
                this->m_mutex->unlock();
        ...
        //2.Read data from ALSA input device.
        while(1)
        {
            ...
            if(nRet<0)
            {
                if(nRet==-EPIPE)
                {
                    qDebug()<<"<AudioCapture>:Buffer Overrun";
                ...
                //>0,a positive number of frames actually read.
                break;
            }
        }

        //3.put buffer into queue.
        this->m_mutex->lock();
